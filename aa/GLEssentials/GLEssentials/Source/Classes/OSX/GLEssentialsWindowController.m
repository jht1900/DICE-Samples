...
- (void) goFullscreen
{
    ...
    _fullscreenWindow.contentView = self.window.contentView;
...
- (void) goWindow
{
    ...
    (self.window.contentView).frame = viewRect;

    // Set controller to the standard window so that all input will go to
    // this controller (self)
    self.window = _standardWindow;

    // Set the content of the orginal window to the view
    self.window.contentView = _fullscreenWindow.contentView;
